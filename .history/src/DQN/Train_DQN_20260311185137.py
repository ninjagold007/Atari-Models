...
class DQNTrainer:
    def __init__(self, num_envs=hp.NUM_ENVS):
        ...
        self.left_count = np.zeros(num_envs)   
        self.right_count = np.zeros(num_envs)  
    ...
    def select_actions(self):
        ...
        valid_states = [s for s in self.current_states if s is not None]
        state_batch = torch.cat(valid_states, dim=0)        
    ...
    def optimize_model(self):
        ...
        done_batch = torch.cat(batch.done, dim=0).to(device)

        # ~ inverts bits, so non_final_mask is True for non-final states
        non_final_mask = ~done_batch.squeeze(1) 
        non_final_next_states = torch.cat([s for s, d in zip(batch.next_state, done_batch) if not d.item()], dim=0).to(device) \
        ...
        while finished_episode_total < hp.TOTAL_FINISHED_EPISODES_TO_RUN:
            ...
            if done_indices:
                ...
                for i in done_indices:
                    ...
                    print(f"Left: {self.left_count.tolist()}, Right: {self.right_count.tolist()}")
