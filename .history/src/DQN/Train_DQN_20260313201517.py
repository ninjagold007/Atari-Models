...
# print("Using device:", device) #gpu or cpu
# if device.type == "cuda":
...
class DQNTrainer:
    def __init__(self, num_envs=hp.NUM_ENVS):
        ...
        self.left_count = np.zeros(num_envs)  #TODO: make each each env and reset it
        self.right_count = np.zeros(num_envs)  #TODO: make each each env and reset it
        self.losses = []
        self.mean_q_values = []
        self.td_errors = []
    ...
    def _epsilon(self):
        eps = hp.EPS_END + (hp.EPS_START - hp.EPS_END) * math.exp(-1. * self.global_steps / hp.EPS_DECAY)
        self.eps_tracker = np.append(self.eps_tracker, eps)
        return eps
    ...
    def select_actions(self):
        ...
        with torch.no_grad():
            ...
            self.mean_q_values = np.append(self.mean_q_values, mean_q)
    ...
    def optimize_model(self):
        ...
        #state _batch is the current state
        state_batch = torch.cat(batch.state, dim=0)
        #action_batch is the action taken in that state
        action_batch = torch.cat(batch.action, dim=0)
        #reward_batch is the reward received after taking that action
        reward_batch = torch.cat(batch.reward, dim=0)
        #done_batch is a boolean indicating if the episode ended after taking that action
        ...
        # Compute Q(s_t, a), the predicted Q values for the actions taken
        prediction = self.policy_net(state_batch).gather(1, action_batch)

        # Compute V(s_{t+1}) for all next states, the maximum Q-value for each next state
        ...
        target = reward_batch.squeeze(1) + hp.GAMMA * next_state_values
        loss = F.smooth_l1_loss(prediction.squeeze(1), target)
    ...
    def run(self):
                ...
                for i in done_indices:
                    ...
                    if self.episode_rewards[i] > self.best_reward:
                        ...
                        torch.save(self.policy_net.state_dict(),
                                   os.path.join(hp.SAVE_DIR, "policy_net_best3.pth"))
                    ...
                    print(f"Left: {self.left_count[i]}, Right: {self.right_count[i]}")
                    print("score:", self.episode_rewards[i])
                    self.episode_rewards[i] = 0.0

                    self.left_count[i] = 0
                    self.right_count[i] = 0
                    ...
                    self.episode_times = np.append(self.episode_times, time.perf_counter() - env_start[i])
        ...
        make_chart(self, np.array(self.episode_rewards), "Episode Rewards", "Episode", "Reward")
        make_chart(self, moving_avg_reward, "Moving Average Reward", "Episode", "Moving Average Reward (100 episodes)")
        make_chart(self, np.array(self.losses), "Training Loss", "Training Steps", "Loss")
        make_chart(self, np.array(self.episode_lengths), "Episode Lengths", "Finished Episodes", "Length (steps)")
        make_chart(self, np.array(self.eps_tracker), "Epsilon Decay", "Training Steps", "Epsilon Value")
        make_chart(self, np.array(self.mean_q_values), "Mean Q-Values", "Training Steps", "Mean Q-Value")
        make_chart(self, np.array(self.td_errors), "TD Errors", "Training Steps", "Mean TD Error")
