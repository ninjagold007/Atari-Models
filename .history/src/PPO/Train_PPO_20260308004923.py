...
class RolloutBuffer:#PPO uses a rollout buffer to store transitions for multiple steps before updating the policy. This class manages that buffer.
    def __init__(self):
        ...
        self.next_states = []
        self.dones = []
        self.log_probs = []  # For storing log probabilities of actions (used in PPO loss)
        self.values = []     # For storing state values (used in PPO loss)
        self.devices = []    # For storing device information of tensors (optional, can be useful for debugging)
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        self.devices.append(state.device)  # Store the device of the state tensor
        self.log_probs.append(None)  # Placeholder for log probabilities
        self.values.append(None)     # Placeholder for state values
