...
class PPOTrainer:
    ...
    def evaluate_actions(self, states, actions):
        # forward pass gives distribution + value
    ...
    def optimize_model(self):
        for buffer in self.buffers:
            ...
            # bootstrap last value (instead of assuming 0)
            with torch.no_grad():
                next_value = values[-1]

            # GAE advantages + returns (better than returns - values)
            ...
            for _ in range(hp.PPO_EPOCHS):
                # get new log probs + values + entropy from current policy
                ...
                # entropy = encourages exploration (prevents policy collapse)
    ...
    def compute_returns_and_advantages(self, rewards, dones, values, next_value=0):
        ...
        # go backwards through rollout (like usual RL stuff)
        for t in reversed(range(len(rewards))):
            # use next value (bootstrapping)
            if t == len(rewards) - 1:
                next_val = next_value
            else:
                next_val = values[t + 1]

            # TD error (core of GAE)
            td_error = rewards[t] + hp.GAMMA * next_val * (1 - dones[t]) - values[t]

            # GAE formula (lambda controls bias vs variance)
            running_advantage = (
                td_error
                + hp.GAMMA * hp.GAE_LAMBDA * running_advantage * (1 - dones[t])
            )

            advantages[t] = running_advantage

            # returns = advantage + value (standard trick)
    ...
    def run(self): # log_probs from actor, values from critic, rewards and dones from envs, states from stacked frames
        ...
        while finished_episode_total < hp.TOTAL_FINISHED_EPISODES_TO_RUN:
            ...
            while rollout_steps < hp.ROLLOUT_STEPS:#make hp.ROLLOUT_STEPS a multiple of hp.NUM_ENVs for simplicity
                ...
                next_frames, rewards, terminateds, truncateds, _ = self.envs.step(actions.cpu().numpy())                # done is when either terminated or truncated is True
                ...
                for i in range(self.num_envs):
                    ...
                    if self.current_states[i] is not None:
                        # detach so we don’t keep old computation graphs (important for memory + correctness)
                        buffer.add(
